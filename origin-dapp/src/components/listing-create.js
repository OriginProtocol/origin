...
class ListingCreate extends Component {
  ...
  async componentDidMount() {
    ...
    console.log('listing id', this.props.listingAddress)
    if (this.props.listingAddress) {
      try {
        const listing = await origin.listings.get(this.props.listingAddress)
        this.props.updateState({ selectedSchemaType: listing.schemaType })
    ...
    fetch(`schemas/${selectedSchemaType}.json`)
      ...
      .then(schemaJson => {
        ...
        this.props.updateState({
          selectedSchemaType,
          schemaFetched: true,
          step: this.props.step === this.STEP.PICK_SCHEMA?this.STEP.DETAILS:this.props.step,
    ...
    this.props.updateState({
      formListing: {
        ...
        formData: {
          ...
          timeIncrement: this.props.fractionalTimeIncrement,
          slots
        }
      }
    })
  ...
  async onSubmitListing(formListing) {
    ...
    try {
      ...
      const methodName = this.props.isEditMode ? 'updateListing' : 'createListing'
            ...
            {step === this.STEP.AVAILABILITY &&
              <div className="col-md-12 listing-availability">
                <Calendar
                  ...
                  userType="seller"
                  viewType={ this.props.fractionalTimeIncrement }
                  step={ 60 }
