...
class DbToApiMapping(object):
    def __init__(self, db_column_to_api_field_dict):
        ...
        self.api_fields_by_name = {f._name: f for f in six.itervalues(db_column_to_api_field_dict)}
...
def apply_pagination(query, api_pagination, default_start=None, default_num=None):
    start = default_start
    num = default_num
...
def apply_ordering(query, api_ordering, db_to_api_mapping, default_ordering=None):
    api_criteria = api_ordering.criteria if api_ordering and api_ordering.criteria else []
    db_columns = []
    for api_criterion in api_criteria:
        db_column = db_to_api_mapping.db_column_for_api_field_name(api_criterion.field_name)
        if db_column:
