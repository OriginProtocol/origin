...
router.post('/verify', asyncMiddleware( async (req, res) => {
  ...
  const accessTokenResponse = await request.get(constants.FACEBOOK_BASE_GRAPH_URL + '/v3.2/oauth/access_token').query(params)
  const accessToken = accessTokenResponse.body.access_token
  const appSecretProof = crypto.createHmac('sha256', process.env.FACEBOOK_CLIENT_SECRET).update(accessToken).digest('hex')
