...
redis.Multi.prototype.publish = function() { this._command("publish_multi", Array.prototype.slice.call(arguments)); return this; }
redis.RedisClient.prototype.publish_multi = function (key, message, cb) { this._client.publish(key, message); cb(null, "1");}
...
describe('Message Queue', () => {
  ...
  describe("#getMessages()", () => {
    ...
    it ("can get a section of messages", async () => {
      const messages = await mqueue.getMessages(testKey, 0)
      const section_messages = await mqueue.getMessages(testKey, messages[2].msgId)
