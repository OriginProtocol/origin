...
async function transferTokensToTestAccounts(deployer, network) {
  ...
  await token.transfer(lastAccount, await token.balanceOf(accounts[0]), {
    from: contractOwner
  })
  ...
  for (let i = 0; i < accounts.length - 1; i++) {
    await token.transfer(accounts[i], tokensPerAccount * 10 ** decimals, {
      from: lastAccount
    })
    console.log(
      `Transfered ${tokensPerAccount * 10 ** decimals} from ${lastAccount} to ${
        accounts[i]
      }`
    )
  }
