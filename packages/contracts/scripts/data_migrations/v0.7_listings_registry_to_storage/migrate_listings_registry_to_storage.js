...
function Migration(config, dataFile) {
    ...
    this.newListingsRegistryAddress = config.newListingsRegistryAddress;
...
Migration.prototype.getAllListings = async function(numListings) {
    ...
    while(true) {
        let getListingCalls = [];
        let target;
        if ((completed + batchSize) > numListings) {
            target = numListings
        } else {
            target = (completed + batchSize)
        }
        for(let i = completed; i < target; i++) {
            getListingCalls.push(this.getListing(i));
        }
        const listings = await Promise.all(getListingCalls);
        Array.prototype.push.apply(allListings, listings);
        completed = target;
        if (completed == numListings) break;
    }

    const retrievedListings = allListings.filter((el) => el);
...
Migration.prototype.read = async function() {
    ...
    this.contractAddress = this.listingsRegistryAddress;
    this.contract = new this.web3.eth.Contract(listingsRegistryAbi, this.contractAddress);
...
Migration.prototype.write = async function() {
    ...
    this.web3.setProvider(this.provider);
    this.contractAddress = this.newListingsRegistryAddress;
    this.contract = new this.web3.eth.Contract(newListingsRegistryAbi, this.contractAddress);
