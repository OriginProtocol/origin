...
function Verifications(props) {
  ...
  const verificationRewardTypes = ['Email', 'Profile', 'Phone', 'Twitter', 'Airbnb', 'Facebook', 'Google', 'Airbnb', 'Facebook', 'Google']
  const verificationActions = activeCampaign.actions.filter(action => verificationRewardTypes.includes(action.type))
  return (
    <div className={`container growth-verifications ${isMobile ? 'mobile' : ''}`}>
      <div>
        <Link
