...
import VerifyLinkedinMutation from 'mutations/VerifyLinkedin'
import query from 'queries/LinkedinAuthUrl'

class LinkedinAttestation extends Component {
  constructor(props) {
    super(props)
    this.state = {
      stage: 'GenerateCode',
  ...
  render() {
    ...
    const redirect = isMobile
      ? encodeURIComponent(`${origin}${pathname}#/profile/linkedin`)
      : null

    return (
      <Modal
        className={`attestation-modal linkedin${
      ...
      >
        ...
        >
          {({ data }) => {
            const authUrl = get(data, 'identityEvents.LinkedinAuthUrl')
            return (
              <div>
  ...
  renderGenerateCode({ authUrl, redirect }) {
    return (
      <>
        ...
        <div className="help">
          <fbt desc="LinkedinAttestation.verify.explanation">
            Other users will know that you have a verified LinkedIn account, but
  ...
  renderVerifyButton({ authUrl, redirect }) {
    ...
    return (
      <Mutation
        mutation={VerifyLinkedinMutation}
        onCompleted={res => {
          const result = res.verifyLinkedin
          if (result.success) {
            this.setState({
              stage: 'VerifiedOK',
              data: result.data,
...
export default withRouter(LinkedinAttestation)
