...
import VerifyGithubMutation from 'mutations/VerifyGithub'
import query from 'queries/GithubAuthUrl'

class GithubAttestation extends Component {
  constructor(props) {
    super(props)
    this.state = {
      stage: 'GenerateCode',
  ...
  render() {
    ...
    const redirect = isMobile
      ? encodeURIComponent(`${origin}${pathname}#/profile/github`)
      : null

    return (
      <Modal
        className={`attestation-modal github${
      ...
      >
        ...
        >
          {({ data }) => {
            const authUrl = get(data, 'identityEvents.GithubAuthUrl')
            return (
              <div>
  ...
  renderGenerateCode({ authUrl, redirect }) {
    return (
      <>
        <h2>
          <fbt desc="GithubAttestation.verify">Verify your GitHub Account</fbt>
        </h2>
        {this.state.error && (
          <div className="alert alert-danger mt-3">{this.state.error}</div>
        )}
        <div className="help">
          <fbt desc="GithubAttestation.verify.explanation">
            Other users will know that you have a verified GitHub account, but
  ...
  renderVerifyButton({ authUrl, redirect }) {
    ...
    return (
      <Mutation
        mutation={VerifyGithubMutation}
        onCompleted={res => {
          const result = res.verifyGithub
          if (result.success) {
            this.setState({
              stage: 'VerifiedOK',
              data: result.data,
  ...
  renderVerifiedOK() {
    return (
      <>
        <h2>
          <fbt desc="GithubAttestation.verified">GitHub account verified!</fbt>
        </h2>
...
export default withRouter(GithubAttestation)
