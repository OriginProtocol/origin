...
import VerifyKakaoMutation from 'mutations/VerifyKakao'
import query from 'queries/KakaoAuthUrl'

class KakaoAttestation extends Component {
  constructor(props) {
    super(props)
    this.state = {
      stage: 'GenerateCode',
  ...
  render() {
    ...
    const redirect = isMobile
      ? encodeURIComponent(`${origin}${pathname}#/profile/kakao`)
      : null

    return (
      <Modal
        className={`attestation-modal kakao${
      ...
      >
        ...
        >
          {({ data }) => {
            const authUrl = get(data, 'identityEvents.KakaoAuthUrl')
            return (
              <div>
  ...
  renderGenerateCode({ authUrl, redirect }) {
    return (
      <>
        <h2>
          <fbt desc="KakaoAttestation.verify">Verify your KaKao Account</fbt>
        </h2>
        {this.state.error && (
          <div className="alert alert-danger mt-3">{this.state.error}</div>
        )}
        <div className="help">
          <fbt desc="KakaoAttestation.verify.explanation">
            Other users will know that you have a verified KaKao account, but
  ...
  renderVerifyButton({ authUrl, redirect }) {
    ...
    return (
      <Mutation
        mutation={VerifyKakaoMutation}
        onCompleted={res => {
          const result = res.verifyKakao
          if (result.success) {
            this.setState({
              stage: 'VerifiedOK',
              data: result.data,
  ...
  renderVerifiedOK() {
    return (
      <>
        <h2>
          <fbt desc="KakaoAttestation.verified">KaKao account verified!</fbt>
        </h2>
...
export default withRouter(KakaoAttestation)
