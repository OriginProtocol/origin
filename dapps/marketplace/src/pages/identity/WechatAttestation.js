...
import VerifyWechatMutation from 'mutations/VerifyWechat'
import query from 'queries/WechatAuthUrl'

class WechatAttestation extends Component {
  constructor(props) {
    super(props)
    this.state = {
      stage: 'GenerateCode',
  ...
  render() {
    ...
    const redirect = isMobile
      ? encodeURIComponent(`${origin}${pathname}#/profile/wechat`)
      : null

    return (
      <Modal
        className={`attestation-modal wechat${
      ...
      >
        ...
        >
          {({ data }) => {
            const authUrl = get(data, 'identityEvents.WechatAuthUrl')
            return (
              <div>
  ...
  renderGenerateCode({ authUrl, redirect }) {
    return (
      <>
        <h2>
          <fbt desc="WechatAttestation.verify">Verify your WeChat Account</fbt>
        </h2>
        {this.state.error && (
          <div className="alert alert-danger mt-3">{this.state.error}</div>
        )}
        <div className="help">
          <fbt desc="WechatAttestation.verify.explanation">
            Other users will know that you have a verified WeChat account, but
  ...
  renderVerifyButton({ authUrl, redirect }) {
    ...
    return (
      <Mutation
        mutation={VerifyWechatMutation}
        onCompleted={res => {
          const result = res.verifyWechat
          if (result.success) {
            this.setState({
              stage: 'VerifiedOK',
              data: result.data,
  ...
  renderVerifiedOK() {
    return (
      <>
        <h2>
          <fbt desc="WechatAttestation.verified">WeChat account verified!</fbt>
        </h2>
...
export default withRouter(WechatAttestation)
