#!/bin/sh

rm ./experimental/origin-admin/package-lock.json
rm ./experimental/origin-dapp2/package-lock.json
rm ./experimental/origin-eventsource/package-lock.json
rm ./experimental/origin-graphql-simple-demo/package-lock.json
rm ./experimental/origin-graphql/package-lock.json
rm ./experimental/origin-ipfs/package-lock.json
rm ./experimental/origin-services/package-lock.json
rm ./experimental/origin-validator/package-lock.json
rm ./origin-contracts/package-lock.json
rm ./origin-dapp-creator-client/package-lock.json
rm ./origin-dapp-creator-server/package-lock.json
rm ./origin-dapp/package-lock.json
rm ./origin-discovery/package-lock.json
rm ./origin-faucet/package-lock.json
rm ./origin-growth/package-lock.json
rm ./origin-ipfs-proxy/package-lock.json
rm ./origin-js/package-lock.json
rm ./origin-linking/package-lock.json
rm ./origin-messaging/package-lock.json
rm ./origin-mobile/package-lock.json
rm ./origin-notifications/package-lock.json
rm ./origin-token/package-lock.json
rm ./origin-token-transfer-client/package-lock.json
rm ./origin-token-transfer-server/package-lock.json
rm ./package-lock.json
