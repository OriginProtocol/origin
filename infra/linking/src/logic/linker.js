...
class Linker {
  ...
  async findUnexpiredCode(code) {
    return db.LinkedToken.findAll({
      where: { code: code, codeExpires: { [Op.gte]: new Date() } }
    })
  }
  ...
  async getWalletNotification(walletToken) {
    return await db.WalletNotificationEndpoint.findOne({
      where: { walletToken }
    })
  }
  ...
  async handleSessionMessages(
    ...
    const {
      ...
      sessionToken,
      lastMessageId
    } = await this.initClientSession(clientToken, _sessionToken, _lastMessageId)
    if (initMsg) {
      messageFn(initMsg)
    }
  ...
  sendSessionMessage(linkedObj, sessionToken, type, data) {
    return this.messages.addMessage(linkedObj.clientToken, {
      type,
  ...
  sendNotify(notify, msg, data = {}) {
    if (notify) {
      if (notify.deviceType == EthNotificationTypes.APN && this.apnProvider) {
        ...
        notify.deviceType == EthNotificationTypes.FCM &&
  ...
  async generateCode(
    ...
    if (!linkedObj.linked) {
      ...
      linkedObj.code = code
      linkedObj.codeExpires = new Date(
        new Date().getTime() + CODE_EXPIRATION_TIME_MINUTES * 60 * 1000
      )
    ...
    if (linkedObj.code && notifyWallet) {
      this.messages.addMessage(notifyWallet, {
        type: MessageTypes.LINK_REQUEST,
        data: { code: linkedObj.code }
      })
    }
    return {
      clientToken,
  ...
  async walletCalled(walletToken, callId, linkId, sessionToken, result) {
    const links = await db.LinkedToken.findAll({
      where: { walletToken, linked: true }
    })
  ...
  async registerWalletNotification(
    walletToken,
    ethAddress,
    deviceType,
    deviceToken
  ...
  async ethNotify(receivers) {
    for (const [_ethAddress, notifyObj] of Object.entries(receivers)) {
      ...
      const notify = await db.WalletNotificationEndpoint.findOne({
        where: { ethAddress }
      })
