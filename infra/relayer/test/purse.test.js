...
const { initStandardSubproviders } = require('@origin/web3-provider')
...
// Should use @origin/web3-provider?
;[false, true].forEach(async function (useWeb3Provider) {

  // Used for testing a second round using web3-provider
  let desc = ''
  let mnemonicMod = ''
  if (useWeb3Provider) {
    await initStandardSubproviders(web3, { ethGasStation: true })
    desc = ' (with @origin/web3-provider)'
    mnemonicMod = ' mod'
  }
  ...
  describe(`Purse${desc}`, () => {
    ...
    it('initialize and autofund accounts', async () => {
      const childCount = 3
      const purse = new Purse({ web3, mnemonic: MNEMONIC_ONE + mnemonicMod, children: childCount, autofundChildren: true })
      await purse.init()
    ...
    it('initialize the same accounts with the same mnemonic', async () => {
      ...
      const purseOne = new Purse({ web3, mnemonic: MNEMONIC_ONE + mnemonicMod, children: childCount })
      await purseOne.init()

      const purseTwo = new Purse({ web3, mnemonic: MNEMONIC_ONE + mnemonicMod, children: childCount })
      await purseTwo.init()

      const purseThree = new Purse({ web3, mnemonic: MNEMONIC_TWO + mnemonicMod, children: childCount })
    ...
    it('sends transactions from children', async () => {
      const purse = new Purse({ web3, mnemonic: MNEMONIC_ONE + mnemonicMod, children: 2 })
      await purse.init()
    ...
    it('keeps persistent and accurate count of nonce', async () => {
      const purseOne = new Purse({ web3, mnemonic: MNEMONIC_ONE + mnemonicMod, children: 2 })
      await purseOne.init()
      ...
      const purseTwo = new Purse({ web3, mnemonic: MNEMONIC_ONE + mnemonicMod, children: 2 })
      await purseTwo.init()

      // Third instance should not have Redis
      const purseThree = new Purse({
        web3,
        mnemonic: MNEMONIC_ONE + mnemonicMod,
        children: 2,
    ...
    it('should init once master account is funded and autofund children', async () => {
      ...
      const purse = new Purse({
        web3,
        mnemonic: MNEMONIC_THREE + mnemonicMod,
        children: childCount,
    ...
    it.skip('should be able to handle a large volume of transactions', async function () {
      ...
      const purse = new Purse({
        web3,
        mnemonic: MNEMONIC_FOUR + mnemonicMod,
        children: childCount,
        autofundChildren: true,
    ...
    it('onReceipt callbacks are utilized', () => {
      ...
      return new Promise(async (resolve) => {
        ...
        const purse = new Purse({
          web3,
          mnemonic: MNEMONIC_FIVE + mnemonicMod,
          children: childCount,
          autofundChildren: true
        })
        await purse.init()
    ...
    it('can drain child accounts', async function () {
      ...
      const purse = new Purse({
        web3,
        mnemonic: MNEMONIC_SIX + mnemonicMod,
        children: childCount,
        autofundChildren: true
      })
      await purse.init()
      ...
      purse.setAutofundChildren(false)
    ...
    it('should timeout when it cannot get an account', async function () {
      ...
      const purse = new Purse({
        web3,
        mnemonic: MNEMONIC_SEVEN + mnemonicMod,
    ...
    it('should be able to fund multiple children from process loop', async function () {
      this.timeout(60000)
      ...
      await purse.init()
      assert(purse.children.length === childCount, 'invalid amoount of children')
      const masterAddress = purse.masterWallet.getChecksumAddressString()
      ...
      if (masterBalance.eq(ZERO)) {
        const masterFundValue = ONE_ETHER.mul(new BN(childCount))
        // fund the master address
        const tx = await web3.eth.sendTransaction({
          to: masterAddress,
          from: Funder,
          value: masterFundValue,
          gas: 21000,
          gasPrice: TWO_GWEI
        })
        ...
        masterBalance = await getBalance(web3, masterAddress)
        assert(masterBalance.eq(masterFundValue))
      }

      // Give it a bit to process and fund the children
      await wait(3000)

      for(let i = 0; i < childCount; i++) {
        const child = purse.children[i]
        const childBal = await getBalance(web3, child)
        assert(childBal.gt(ZERO), `balance should be >0, funding failed`)
      }
      ...
      // Fund them again
      purse.setAutofundChildren(true)

      // Give it a bit to process
      await wait(10000)

      // Verify they're funded again
      for(let i = 0; i < childCount; i++) {
        const child = purse.children[i]
        const childBal = await getBalance(web3, child)
        assert(childBal.gt(ZERO), `balance should be >0, refunding failed`)
      }
