...
function checkExpectedState(state, expectedState) {
  ...
  expect(state.actions.length).to.equal(13) // Note: adjust based on number of rules.
...
describe('Apollo adapter - Feb 2020 campaign', () => {
  before(async () => {
    ...
    createCampaignRules(feb2020Config, this)
    ...
    expect(this.crules.levels[1].rules.length).to.equal(11) // Note: adjust based on number of rules.
  ...
  it(`It should fail with an invalid additional lock condition`, async () => {
    ...
    const modifyTwitterShareRule = (modificationCallback) => {
      feb2020Config.levels['1'].rules = feb2020Config
        .levels['1']
        .rules
        .map(rule => {
    ...
    // modify feb2020 config an enter an invalid rule id
    ...
    try {
      createCampaignRules(feb2020Config, this.campaignStart, this.campaignEnd)
