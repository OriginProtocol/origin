...
describe('Apollo adapter - September campaign', () => {
  before(async () => {
    ...
    createCampaignRules(septemberConfig, this)
  ...
  it(`It should fail with an invalid additional lock condition`, async () => {
    ...
    const modifyTwitterShareRule = (modificationCallback) => {
      septemberConfig.levels['2'].rules = septemberConfig
        .levels['2']
        .rules
        .map(rule => {
    ...
    // modify september config an enter an invalid rule id
    ...
    try {
      createCampaignRules(septemberConfig, this.campaignStart, this.campaignEnd)
    ...
    this.events.push(...[
      {
        ...
        customId: '71a2ce62e47c38d5edd23a96edb193f5',
        data: {
          twitterProfile: {
            verified: false,
