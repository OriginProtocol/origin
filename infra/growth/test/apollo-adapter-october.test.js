...
describe('Apollo adapter - October campaign', () => {
  before(async () => {
    ...
    createCampaignRules(octoberConfig, this)
  ...
  it(`Adapter completed level 0`, async () => {
    ...
    this.expectedState.FacebookShare15.status = 'Active'
  ...
  it(`Adapter completed level 1`, async () => {
    ...
    this.expectedState.TwitterShare15.status = 'Inactive'
  ...
  it(`Adapter at level 2 with a mobile install`, async () => {
    ...
    this.expectedState.rewardEarned = { amount: '180000000000000000000', currency: 'OGN' }
  ...
  it(`It should fail with an invalid additional lock condition`, async () => {
    ...
    const modifyTwitterShareRule = (modificationCallback) => {
      octoberConfig.levels['2'].rules = octoberConfig
        .levels['2']
        .rules
        .map(rule => {
          if (rule.id === 'TwitterShare15') {
            modificationCallback(rule)
          }
          return rule
        })
    }
    // modify october config an enter an invalid rule id
    ...
    try {
      createCampaignRules(octoberConfig, this.campaignStart, this.campaignEnd)
  ...
  it(`It should unlock TwitterShare and TwitterFollow if Twitter attestation is present`, async () => {
    ...
    this.expectedState.TwitterShare15.status = 'Active'
  ...
  it(`It should reward OGN for content shared on twitter`, async () => {
    ...
    this.expectedState.rewardEarned = { amount: '290000000000000000000', currency: 'OGN' }
    this.expectedState.TwitterShare15.status = 'Completed'
    this.expectedState.TwitterShare15.rewardEarned = { amount: tokenToNaturalUnits(100), currency: 'OGN' }
  ...
  it(`It should show facebook share completed `, async () => {
    ...
    this.expectedState.FacebookShare15.status = 'Completed'
    this.expectedState.FacebookShare15.rewardEarned = { amount: '0', currency: 'OGN' }
