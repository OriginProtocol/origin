...
class MessagingProvider extends Component {
  ...
  componentDidUpdate(prevProps) {
    const { messages, messagingEnabled, messagingInitialized, web3Account } = this.props
    ...
    if (!messages.find(({ hash }) => hash === 'origin-welcome-message')) {
      ...
      let message = {
        ...
        content: this.props.intl.formatMessage(this.intlMessages.welcomeMessage),
    ...
    if (messagingEnabled !== prevProps.messagingEnabled) {
      ...
      let message = {
        ...
        content: this.props.intl.formatMessage(this.intlMessages.congratsMessage),
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(injectIntl(MessagingProvider)))
